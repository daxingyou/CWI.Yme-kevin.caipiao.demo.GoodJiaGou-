...
namespace Yme.Util
{
    public static class FileOperateUtil
    {
        ...
        /// 创建文件
        /// </summary>
        /// <param name="path">文件地址，从本站根目录开始相对地址  前面不需要加"/"</param>
        /// <param name="html">文件内容</param>
        /// <returns></returns>
        public static bool CreateHtmlFile(string path, string html, bool isAll = true)
        {
            bool flag = false;

            try
            {
                path = path.IndexOf(":") >= 1 ? path : string.Format("{0}{1}", System.Web.HttpContext.Current.Request.PhysicalApplicationPath);
                ...
                if (!isAll)
                {
                    ...
                    sw.WriteLine("</head>");
                    sw.WriteLine("<body>");
                    sw.WriteLine(HttpUtility.UrlDecode(html, System.Text.Encoding.UTF8));
                    sw.WriteLine("</body>");
                    sw.WriteLine("</html>");
                }
                else
