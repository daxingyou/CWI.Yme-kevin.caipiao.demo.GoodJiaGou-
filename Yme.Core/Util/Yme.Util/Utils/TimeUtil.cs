...
namespace Yme.Util
{
    ...
    public class TimeUtil
    {
        ...
        /// <returns></returns>
        public static List<string> GetDateList(DateTime beginDate, DateTime endDate, int dateFormat = 1)
        {
            var dateList = new List<string>();
            for (DateTime? t = beginDate; t <= endDate; t = t.Value.AddDays(1))
            {
                if (dateFormat == 1)
                {
                    dateList.Add(t.ToDateString());
                }
                else if (dateFormat == 2)
                {
                    dateList.Add(t.Value.ToShorDateFormatString());
                }
                else if (dateFormat == 3)
                {
                    dateList.Add(t.ToMonthPointDayString());
                }
                else
                {
                    dateList.Add(t.ToDateString());
        ...
        /// <summary>
        /// 返回如下格式：04-15（星期六）15:15
        /// </summary>
        /// <param name="timestamp"></param>
        /// <returns></returns>
        public static string GetWeekDateTime(long timestamp)
        {
            var dt = TimeUtil.GetDateTime(timestamp);
            return string.Format(dt.ToString("MM-dd （{0}）HH:mm"), GetChineseWeekDay(dt.Year, dt.Month, dt.Day));
        }
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static string GetWeekDateTime(DateTime dt)
        {
            return string.Format(dt.ToString("MM-dd （{0}）HH:mm"), GetChineseWeekDay(dt.Year, dt.Month, dt.Day));
        }
