...
namespace Yme.Data
{
    public class DatabaseCommon
    {
        ...
        public static StringBuilder SelectSql<T>(int Top) where T : new()
        {
            ...
            {
                strSql = "SELECT {1} FROM {2} WHERE 1=1 LIMIT 0,{0} ";
            }
            else
            {
                strSql = "SELECT top {0} {1} FROM {2} WHERE 1=1 ";
            }
        ...
        public static StringBuilder SelectSql(string tableName, int Top)
        {
            ...
            if (DbHelper.DbType == DatabaseType.MySql)
            {
                sql = "SELECT * FROM {1} WHERE 1=1 LIMIT 0,{0} ";
            }
            else
            {
                sql = "SELECT top {0} * FROM {1} WHERE 1=1 ";
            }

            strSql.AppendFormat(sql, Top, tableName);
            return strSql;
        }
        /// <summary>
        /// 拼接 查询条数 SQL语句
