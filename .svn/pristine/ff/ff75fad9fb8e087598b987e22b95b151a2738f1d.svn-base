        ...
        public string ToJson()
        {
            ...
            if (!this.Alert.IsEmpty)
            {
                if (!string.IsNullOrEmpty(this.Alert.Body)
                    ...
                    && !this.Alert.EmailId.HasValue
                    && !string.IsNullOrEmpty(this.Alert.OrderNo)
                    && !this.Alert.PushMsgType.HasValue
                    )
                {
                ...
                else
                {
                    ...
                    if (this.Alert.EmailId.HasValue)
                    {
                        jsonAlert["email_id"] = new JValue(this.Alert.EmailId);
                    }
                    if (string.IsNullOrEmpty(this.Alert.OrderNo))
                    {
                        jsonAlert["is_chain"] = new JValue(this.Alert.OrderNo);
                    }
                    if (this.Alert.PushMsgType.HasValue)
                    {
                        jsonAlert["msg_type"] = new JValue(this.Alert.PushMsgType);
                    }
                    aps["alert"] = jsonAlert;
                }
            }
