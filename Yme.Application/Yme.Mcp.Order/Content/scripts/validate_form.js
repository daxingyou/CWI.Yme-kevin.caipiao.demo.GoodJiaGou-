...
$.fn.validateForm = function(config) {
	...
	function ValidateBase() {
		 ...
		 * @param  {[type]} el [元素]
		 * @param  {[type]} v  [值]
		 * @param  {[type]} c  [验证规则 c:{min:1} OR c:{min:1,max:10}]
		 * @return {[type]}    集成两种验证,验证长度 OR 验证最小和最大数之间
		 */
		this.len = function(el, v, c) {
				...
				var conf_mess = {
					empty: "不能为空",
					len: "应输入" + c.len.min+ "位",
					min: "最少应输入" + c.len.min + "个字符",
					max: "最多应输入" + c.len.max + "个字符",
					rang: "应输入" +c.len.min+ "到" +c.len.max+ "个字符"
				}
				if (v.length) {
					if (typeof c.len.max !== "undefined") {
						if (c.len.max && c.len.min) {
		...
		this.space = function(el, v, c){
			...
			if (typeof c.space.mess !== "undefined") {
				mess = c.space.mess;
			}
			Rander.setRander($(el)).setDecorate(["border", "color"]);
			this.render({
	...
	var Parse = {
		...
		space: function(el, conf, aname, avalue){
			conf.space = {};
			if (avalue) {
				avalue = avalue.split(",");
				if (avalue.length === 2) {
					conf.space.mess = avalue[1];
				}
				conf.space.require = Number(avalue[0]);
			}
			return conf;
		}
	}
