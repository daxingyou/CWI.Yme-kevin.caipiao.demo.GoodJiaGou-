...
namespace Yme.Mcp.WinServ.Busy.TimerCtrl
{
    ...
    /// 业绩日报定时服务
    /// </summary>
    public class OrderDayRptTimer : BaseTimer
    {
        //执行分钟
        ...
        public override void Timer_Elapsed()
        {
            ...
            if (now == runTimer)
            {
                LogUtil.Info("开始生成订单业绩日报...");

                SingleInstance<ReportBLL>.Instance.CreateOrderDayReport(now.AddDays(-1).Date);
            }
            ...
            //    SingleInstance<ReportBLL>.Instance.CreateOrderDayReport(d.ToDate());
            //}
        }
    }
}
